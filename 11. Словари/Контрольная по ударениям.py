...
#Мое решение
'''
Первый вариант проходил все тесты кроме 2ух, потому что не правильно понял условие задачи.
например:
Если в словаре есть Zn, а Петя написал zN то мой код не считал это ошибкой.
'''
def isupchar(str):
    count = 0
    for j in str:
...
for j in str:
    base = j.lower()
    if (base in d and j not in d[base] or isupchar(j) != 1):
        count += 1
print(count)
